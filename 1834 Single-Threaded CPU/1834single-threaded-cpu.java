...
class Pair{
    ...
    public Pair(int enqueueTime,int processingTime,int index){
...
class Solution {
    public int[] getOrder(int[][] tasks) {
        ...
        int index = 0;
        for(int[] task:tasks){
            pairs.add(new Pair(task[0],task[1],index++));
        }
        // Sort them by enqueue time since question doesn't have it in sorting order
        pairs.sort(Comparator.comparingInt(pair -> pair.enqueueTime));
        // Now we need min Heap to get the itens with least processing time
        // and if processing time is the same then tie-breaker with the one having min index
        ...
        index = 0;
        int currentTime = pairs.get(0).enqueueTime; // CPU will not get any task before the first tasks enqueue time hence this.
        int[] result = new int[tasks.length];
        int resultCtr = 0;
        while(index < pairs.size() || !minHeap.isEmpty()){
            // Enqueue all tasks which could be started before the currentTime or on currentTime
            while(index < pairs.size() && currentTime >= pairs.get(index).enqueueTime){
                minHeap.offer(pairs.get(index));
                index++;
            }
            // Now we just need to move our time to the next window since we have all tasks in heap which should hav started before the currentTime
            if(!minHeap.isEmpty()){
                Pair polled = minHeap.poll();
                currentTime += polled.processingTime;
